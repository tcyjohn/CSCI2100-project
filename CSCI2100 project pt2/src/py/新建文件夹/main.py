...
if __name__ == "__main__":
    ...
    b_values = [100, 200]  # Different block sizes
    T_values = [1, 4, 16, 128, 512]  # Different relative times for secStore access
    H_data = {B: {}}  # Dictionary to store H data for different b and T
    ...
    for b in b_values:
        H_data[B][b] = []
        for T in T_values:
            H = external_merge_sort(B, b, N, T, input_file, output_file)
            H_data[B][b].append(H)
    
    plot_H_vs_T(H_data, T_values, B, b_values)
